...
def load_shaders(fragment_filename: str | None, vertex_filename: str | None = None):
    """ Load shaders from file, or from the shaders directory"""

    if fragment_filename is None:
        frag_code = resources.read_text("hssss.shaders", "default_fragment.glsl")
        ...
        if os.path.exists(fragment_filename):
            ...
            frag_code = resources.read_text("hssss.shaders", fragment_filename + ".glsl")

    if vertex_filename is None:
        vertex_code = resources.read_text("hssss.shaders", "default_vertex.glsl")
        ...
        if os.path.exists(vertex_filename):
            ...
            vertex_code = resources.read_text("hssss.shaders", fragment_filename + ".glsl")
